...
export class Transport {
    ...
    async send(path, init = {}) {
        ...
        try {
            ...
            throw new SyncError('Could not reach the server. It is only reachable on the home network.');
        }
    ...
    async pair(app, code, deviceName, user) {
        let response;
        try {
    ...
    async pull(app, since, limit) {
        const response = await this.send(`/api/${encodeURIComponent(app)}/changes?since=${since}&limit=${limit}`);
        return (await response.json());
    }
