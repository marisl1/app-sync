...
import { acceptsRemote, metaKey, nextMeta, remoteMeta, settledMeta } from './meta.js';
 ...
 */
export function markInTransaction(transaction, metaStore, collection, id, now, deleted = false) {
    const store = transaction.objectStore(metaStore);
    const read = store.get(metaKey(collection, id));
    read.onsuccess = () => {
        store.put(nextMeta(read.result, collection, id, now, deleted));
    };
 ...
 */
function announceOnCommit(transaction) {
...
export class IdbAdapter {
    ...
    cacheStore(store) {
        ...
        return {
            has: async (hash) => {
                ...
                const row = await request(transaction.objectStore(store).get(hash));
    ...
    async pending(limit) {
        ...
        const all = await (async () => {
            ...
            const rows = request(transaction.objectStore(metaStore).getAll());
            await committed(transaction);
            return rows;
        })();
        ...
        const reads = dirty.map((meta) => meta.deleted || !this.syncs(meta.collection)
            ? Promise.resolve(undefined)
            : request(transaction.objectStore(meta.collection).get(meta.id)));
     ...
     */
    async detach(collection, record) {
        ...
        transaction.objectStore(store).put({ hash, dataUrl: value });
        await committed(transaction);
        const marker = { $blob: hash, mime: decoded.mime };
        return {
            payload: JSON.stringify({ ...record, [field]: marker }),
            blobHashes: [hash],
        };
    }
    /**
    ...
    async apply(changes) {
        ...
        const transaction = db.transaction(blobStore === undefined ? [...stores, metaStore] : [...stores, metaStore], 'readwrite');
        ...
        for (const change of changes) {
            ...
            read.onsuccess = () => {
                ...
                meta.put(remoteMeta(change.collection, change.id, change.seq, change.updatedAt, change.deleted));
            };
        }
        await committed(transaction);
    }
    ...
    async loadState() {
        ...
        const stored = await request(transaction.objectStore(this.options.stateStore).get(STATE_KEY));
        await committed(transaction);
        if (stored === undefined) {
            return null;
        }
